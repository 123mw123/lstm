...
# Model path.
MODEL_PATH = "C:\\Users\Sai Teja\Desktop\ELL888-RNN\\model.ckpt\\"

# Summary directory.
SUMMARY_PATH = "C:\\Users\Sai Teja\Desktop\ELL888-RNN\\logs\\"
    ...
    with tf.device('/cpu:0'):
        ...
        with graph.as_default():
            ...
            def lstm_cell():
                ...
                return tf.contrib.rnn.BidirectionalGridLSTMCell(NUM_HIDDEN, state_is_tuple=True)
        ...
        with tf.Session(config=config, graph=graph) as session:
            ...
            log_o = session.run(logits,feed_dict = test_feed)
            #print(log_o)
            #print("===================")
            #print(log_o.shape)
            #p = log_o[:,0,:]
            #print(p.shape)
            #p = np.reshape(p,[p.shape[0],28])
            #print(p.shape)
            #print(np.argmax(p,axis=1))
            #print(np.argmax(p,axis=1).shape)
            bound = {}
            for j in range(log_o.shape[1]):
               
                if(test_texts[j]+'_'+str(int(j/10)) not in bound.keys()):
                    #print(j)
                    bound[test_texts[j]+'_'+str(int(j/10)) ]=[]
                p = log_o[:,j,:]
                #print(p.shape)
                p = np.reshape(p,[p.shape[0],28])
                #print(p.shape)
                ...
                for i in range(p.shape[0]):
                    if(np.argmax(p,axis=1)[i]==0 ):
                        if(i>0 and np.argmax(p,axis=1)[i-1]!=0):  
                            #print(i*512*16000/22050)
                            bound[test_texts[j]+'_'+str(int(j/10))] = bound[test_texts[j]+'_'+str(int(j/10))] + [i*512*16000/22050]

                #print(np.argmax(p,axis=1).shape)
               
            with open('file.txt', 'w') as file:
                file.write(json.dumps(bound))
